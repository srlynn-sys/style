(()=>{
  async function initMusic(){
    ...
    const b=document.createElement('button');b.className='style-music-bubble';b.type='button';b.textContent='♫';b.setAttribute('aria-label','Style Saan Music');
    const l=document.createElement('div');l.className='style-music-label';l.textContent='STYLE SAAN MUSIC';
    ...
    document.body.append(l,b,modal);
    ...
    let songs=[];
    try{if(window.StyleSaanFirebaseReady){const fb=await window.StyleSaanFirebaseReady;const {collection,getDocs}=await import('https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js');const snap=await getDocs(collection(fb.db,'songs'));songs=snap.docs.map(x=>x.data()).filter(x=>x.url)}}catch(e){console.warn('[Style Saan] songs unavailable',e)}
    ...
    b.onclick=()=>{modal.style.display=modal.style.display==='none'?'block':'none';l.textContent=modal.style.display==='block'?'SELECT A SONG':'STYLE SAAN MUSIC';l.classList.add('show');setTimeout(()=>l.classList.remove('show'),1400)};
    document.addEventListener('click',e=>{if(!modal.contains(e.target)&&e.target!==b)modal.style.display='none'});
  }
