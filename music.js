(()=>{
  ...
  function setLabel(text,temp=true){if(!label)return;label.textContent=text;label.classList.add('show');if(temp){clearTimeout(window.__styleMusicTimer);window.__styleMusicTimer=setTimeout(()=>label.classList.remove('show'),2200)}}
  function ensureAudio(){
    if(audio)return audio;audio=document.createElement('audio');audio.id='style-saan-audio';audio.preload='none';audio.setAttribute('playsinline','');audio.style.display='none';document.body.appendChild(audio);
    ...
    audio.addEventListener('error',()=>{bubble?.classList.remove('playing');if(bubble)bubble.textContent='♫';setLabel('သီချင်းဖွင့်မရပါ — Link စစ်ပါ')});return audio;
  }
  async function playSong(index){
    songs=await getSongs();if(!songs.length){setLabel('ADMIN PANEL မှာ Song မရှိသေးပါ');return false}const song=songs[index],url=cleanUrl(song?.url);if(!url){setLabel('Song Link မရှိပါ');return false}current=index;const a=ensureAudio();
    try{a.pause();a.src=url;a.load();await a.play();setLabel(song.name||'STYLE SAAN MUSIC',false);render();return true}catch(e){console.warn('[Style Saan] audio.play failed',e);setLabel('သီချင်းဖွင့်မရပါ — Audio Link စစ်ပါ');return false}
  }
  function render(){if(!list)return;list.innerHTML=songs.length?songs.map((s,i)=>`<button type="button" class="style-song-link" data-index="${i}" style="display:block;width:100%;text-align:left;margin-top:8px;padding:11px 12px;border-radius:12px;border:1px solid #4b3a20;background:${i===current?'#2b2112':'#17130d'};color:#fff;font-size:.82rem;cursor:pointer"><b>${i===current?'🎶':'🎵'} ${esc(s.name||'Song')}</b><br><small style="color:#a99b7b">${i===current?'Playing…':'Play song'}</small></button>`).join(''):'<div style="padding-top:10px;color:#9f947c;font-size:.8rem">Admin Panel → Songs မှာ Chatbox.moe song link ထည့်ပါ။</div>'}
  async function initMusic(){
    ...
    bubble.onclick=async e=>{e.preventDefault();e.stopPropagation();songs=await getSongs();render();if(!songs.length){setLabel('ADMIN PANEL မှာ Song မရှိသေးပါ');return}if(songs.length===1){if(audio&&!audio.paused&&current===0){audio.pause();setLabel('MUSIC PAUSED')}else await playSong(0);return}modal.style.display=modal.style.display==='none'?'block':'none';bubble.classList.toggle('playing',modal.style.display==='block'||(audio&&!audio.paused));setLabel(modal.style.display==='block'?'SELECT A SONG':'STYLE SAAN MUSIC')};
